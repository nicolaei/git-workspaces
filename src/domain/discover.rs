...
/// Walk upward from `start` looking for a `multirepo.toml`, exactly like
...
pub fn find_workspace_root(start: &Path, exists: impl Fn(&Path) -> bool) -> Option<PathBuf> {
    ...
    loop {
        let candidate = current.join("multirepo.toml");
        if exists(&candidate) {
...
mod tests {
    ...
    fn finds_manifest_in_the_starting_directory() {
        let manifest_paths = exists_set(&["/workspace/multirepo.toml"]);
