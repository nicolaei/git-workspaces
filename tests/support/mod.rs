...
//!
//! Runs the real compiled `git-multirepo` binary via `assert_cmd`, not
//! `git_multirepo::run()` in-process — `std::process::ExitCode` can't be
...
impl Workspace {
    ...
    /// Declare a repo in `multirepo.toml`, writing/updating the real file
    ...
    fn write_manifest(&self) {
        ...
        std::fs::write(self.root().join("multirepo.toml"), content).expect("write manifest");
    }

    /// Run `git-multirepo` with the given args from the workspace root.
    pub fn run(&self, args: &[&str]) -> RunResult {
        self.run_from(self.root(), args)
    }

    /// Run `git-multirepo` with the given args from an explicit cwd (e.g.
    /// a nested subfolder).
    pub fn run_from(&self, cwd: &Path, args: &[&str]) -> RunResult {
        let output = Command::cargo_bin("git-multirepo")
            .expect("locate built git-multirepo binary")
            .args(args)
            .current_dir(cwd)
            .output()
            .expect("run git-multirepo");
    ...
    pub fn init_as_git_repo(&self) -> &Self {
        ...
        run_git(self.root(), &["add", "multirepo.toml"]).expect("stage manifest");
        run_git(
            self.root(),
            &[
